...
import { get, run, isAsyncFunction, inWhich, transBool } from './utils';
...
function StoreProvider(props) {
  ...
  const dispatch = async (action, payload, key) => {
    ...
    // {key: "user", type: "setName", payload: {…}}key: "user"payload: {name: "同步修改"}type: "setName"__proto__: Object undefined undefined
    actionAsync = undefined;
    // console.log(get([asyncKey, 'reducers', action.type], initStore));

    if (get([asyncKey, 'reducers', action.type], initStore)) {
      return origin_dispatch(action);
    }
    // console.log(!isAsyncFunction(action)); // true
    // console.log(
...
function useStoreHook() {
  ...
  let _store = {};
  // console.log(Object.keys(state)); // ['user']
  // console.log(initStore);

  Object.keys(state).forEach((key) => {
    let _methods = {};
    const methods = {
      ...get([key, 'methods'], initStore), // key === user
      ...get([key, 'reducers'], initStore),
      ...get([key, 'effects'], initStore),
    };
    // const methods = initStore['user']['effects'];
    console.log(methods); // {setName: ƒ, setNameAsync: ƒ}

    const methodsArr = methods && Object.keys(methods);
    // console.log(methodsArr); // ["setName", "setNameAsync"]

    is(Array, methodsArr) &&
      methodsArr.forEach((item) => {
        // console.log(methods[item].constructor.name);

        // 判断是否是effects 里面的异步函数
        if (
          isAsyncFunction(methods[item]) ||
          inWhich('effects', initStore[key], item)
        ) {
          // console.log('effects', initStore[key], item); //effects user setNameAsync
